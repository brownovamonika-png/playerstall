#!/usr/bin/env python3
"""Recolor Pro Locker image to match varsity locker wood color (Essential Lockers card)."""
...
def main():
    ...
    pro = Image.open(pro_path).convert("RGBA")
    w, h = pro.size
    data = pro.getdata()
    new_data = []
    for item in data:
    ...
    pro.putdata(new_data)
    pro.save(pro_path, "PNG", optimize=True)
    print(f"Saved {pro_path} ({w}x{h})")
